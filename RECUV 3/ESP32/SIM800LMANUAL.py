...
class Modem(object):
    ...
    def initialize(self):
        ...
        if not self.uart:
            ...
            # Setup UART
            self.uart = UART(2, 9600, timeout=1000, rx=self.MODEM_TX_PIN, tx=self.MODEM_RX_PIN)
    ...
    def execute_at_command(self, command, data=None, clean_output=True):
        ...
        commands = {
                    ...
                    'dopost':     {'string':'AT+HTTPACTION=1', 'timeout':20, 'end': '+HTTPACTION'},
                    'getdata':    {'string':'AT+HTTPREAD', 'timeout':3, 'end': 'OK'},
                    'closehttp':  {'string':'AT+HTTPTERM', 'timeout':3, 'end': 'OK'},
                    'closebear':  {'string':'AT+SAPBR=0,1', 'timeout':5, 'end': 'OK'}
        }
    ...
    def http_request(self, url, mode='GET', data=None, content_type='application/json'):
        ...
        self.execute_at_command('inithttp') #INICIALIZAMOS EL MODULO HTTP CON SIM800
        logger.debug('Http request step #1.2 (sethttp)')
        self.execute_at_command('sethttp') #SET HTTP
        ...
        self.execute_at_command('initurl', data=url) #HOST:
        ...
        if mode == 'GET':
            ...
            self.execute_at_command('setcontent', content_type) #CONTENT TYPE
            

            logger.debug('Http request step #2.3 (postlen)')
            self.execute_at_command('postlen', len(data))  #TAM
            

            logger.debug('Http request step #2.4 (dumpdata)')
            self.execute_at_command('dumpdata', data) #VARIABLES
    ...
    #METODOS PARA UTILIZAR DESDE EL MAIN
